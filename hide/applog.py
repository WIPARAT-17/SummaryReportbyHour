...
def get_data_from_api(nod_id, itf_id, job_id, timeout=100, delay=5):
    """
    ดึงข้อมูลสถานะวงจรจาก API ภายนอก (SOAP-based) พร้อมกลไกการลองใหม่จนกว่าจะหมดเวลา
 
    Parameters:
    - nod_id (str): Node ID ของอุปกรณ์
    - itf_id (str): Interface ID ของ Interface
    - job_id (str): ID ของงานปัจจุบันสำหรับ logging
    - timeout (int): ระยะเวลาสูงสุดในการพยายามทั้งหมดเป็นวินาที
    - delay (int): ระยะเวลาที่รอระหว่างการลองใหม่แต่ละครั้งเป็นวินาที
...
<soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"
  ...
  <soap:Body>
    ...
    while True:
        try:
            # ตรวจสอบเวลาที่ใช้ไป ถ้าเกิน timeout ให้หยุด
            elapsed_time = time.time() - start_time
            if elapsed_time > timeout:
                logger.error(f"❌ ไม่สามารถดึงข้อมูลจาก API ได้สำหรับ NodeID: {nod_id}, Interface ID: {itf_id} เนื่องจากหมดเวลา (timeout) ที่ {timeout} วินาที")
                return None

            # ส่ง POST Request ไปยัง API ด้วย timeout สั้นๆ สำหรับแต่ละครั้ง
            resp = requests.post(url, data=body, headers=headers, timeout=5)
            resp.raise_for_status()

            # ... (ส่วนที่เหลือของโค้ดที่ประมวลผล XML Response) ...
            ...
            elapsed_time = time.time() - start_time
            logger.warning(f"⚠️ ดึงข้อมูล NodeID: {nod_id}, Interface ID: {itf_id} ล้มเหลว: {req_e}. (เวลาที่ใช้ไป: {elapsed_time:.2f} วินาที)")
            
            # ตรวจสอบว่ายังมีเวลาพอที่จะลองใหม่หรือไม่
            if elapsed_time + delay > timeout:
                logger.error(f"❌ ไม่สามารถดึงข้อมูลจาก API ได้สำหรับ NodeID: {nod_id}, Interface ID: {itf_id} เนื่องจากหมดเวลา (timeout) ที่ {timeout} วินาที")
                return None
            
            logger.info(f"⏳ กำลังรอ {delay} วินาที ก่อนจะลองใหม่อีกครั้ง...")
            time.sleep(delay)
            
        except (ET.ParseError, json.JSONDecodeError) as e:
            logger.error(f"❌ ข้อผิดพลาดในการประมวลผลข้อมูลสำหรับ NodeID: {nod_id}, Interface ID: {itf_id}: {e}")
            return None
        except Exception as e:
...
def process_file_in_background(file_stream, job_id):
    ...
    try:
        ...
        # ย้ายโค้ดการลบโฟลเดอร์ย่อยมาไว้ใน finally block นี้
        # ซึ่งจะทำงานเมื่อ process_file_in_background จบลงแล้วเท่านั้น
